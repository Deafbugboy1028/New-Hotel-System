...
public class Reservation {
	...
	private ArrayList<Hotel> hotels;// Info for all the hotels //
	...
	private fileIO fileIO; // fileIO variable //
	private String[][] hotelInfo;
	private Date date; // Current date //
	private File file; // Hotel file //
	 ...
	 */
	public Reservation() {
		hotels = new ArrayList<Hotel>();      // Initialise the hotels arrayList//
	...
	public void login() 
	{
		...
		try {
			...
			case 2:
				currentUser = new deskAdmin();
				break;

			case 3:
				currentUser = new Supervisor();
				break;
		...
		currentUser.askQuestions(input, hotels, resInfo);
	...
	public void closeInterface()
	{
		...
		fileIO.writeTo(pw, hotels, resInfo, resInfoFile); // call the fileIO method 'writeTo'
	}
	...
	public void assignInfo(){
		...
		for(int i=2; i<hotelInfo.length; i++){ //moving down the 2D array of hotels,start at row two because all othe info is useless
			...
			if(!hotelInfo[i][0].isEmpty()) //if first comlumn has something in it
			{
